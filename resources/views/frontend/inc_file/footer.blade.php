
    <!-- footer start -->
    <footer class="main-footer">
        <div class="copyright">
            <div class="row">
                <div class="col-md-12">
                    <div class="copyright-col text-center">
                        <p>Copyright ©{{ date('Y'); }} <a href="https://smartbdsoft.com"
                                target="_blank">Smart BD Soft</a> All Rights Reserved</p>
                    </div>
                </div>
            </div>
        </div>
    </footer>

    <!-- jQuery -->
    <script src="{{ asset('frontend') }}/js/jquery.min.js"></script>

    <!-- Bootstrap Core JavaScript -->
    <script src="{{ asset('frontend') }}/js/bootstrap.min.js"></script>

    <!-- All Included JavaScript -->
    <script src="{{ asset('frontend') }}/js/jquery.easing.min.js"></script>
    <script src="{{ asset('frontend') }}/js/scrolling-nav.js"></script>
    <script src="{{ asset('frontend') }}/js/wow.min.js"></script>
    <script src="{{ asset('frontend') }}/js/animated-text.js"></script>
    <script src="{{ asset('frontend') }}/js/slick.min.js"></script>
    <script src="{{ asset('frontend') }}/js/jquery.appear.js"></script>
    <script src="{{ asset('frontend') }}/js/progress-bar-min.js"></script>
    <script src="{{ asset('frontend') }}/js/jquery.waypoints.min.js"></script>
    <script src="{{ asset('frontend') }}/js/jquery.countup.min.js"></script>
    <script src="{{ asset('frontend') }}/js/jarallax.js"></script>
    <script src="{{ asset('frontend') }}/js/lightbox.min.js"></script>
    <script src="{{ asset('frontend') }}/js/dyscrollup.js"></script>
    <script src="{{ asset('frontend') }}/js/jquery.mb.YTPlayer.min.js"></script>
    <script src="{{ asset('frontend') }}/js/jquery.ripples.js"></script>
    <script src="{{ asset('frontend') }}/js/imagesloaded.min.js"></script>
    <script src="{{ asset('frontend') }}/js/jquery.filterizr.min.js"></script>

    <!-- Google map -->
    <script src="{{ asset('frontend') }}/js/google-map.js"></script>
    <script async defer
        src="https://maps.googleapis.com/maps/api/js?key=AIzaSyDdEPAHqgxFK5pioDAB3rsvKchAtXxRGO4&amp;callback=initMap"></script>

    <!-- Custom Js -->
    <script src="{{ asset('frontend') }}/js/main.js"></script>

    <!-- AJAX Contact Form Using -->
    <script src="{{ asset('frontend') }}/js/app.js"></script>

</body>

</html>